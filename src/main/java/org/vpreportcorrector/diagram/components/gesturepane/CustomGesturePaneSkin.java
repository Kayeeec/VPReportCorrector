...
import org.vpreportcorrector.diagram.components.gesturepane.CustomGesturePane.FitMode;
import org.vpreportcorrector.diagram.components.gesturepane.CustomGesturePane.ScrollBarPolicy;
 ...
 * Private API
 * <br>
 * Skin for the {@link CustomGesturePane}, responsible for the following:
 * <ul>
 ...
 * </ul>
 */
final class CustomGesturePaneSkin extends SkinBase<CustomGesturePane> {
    ...
    private final CustomGesturePane pane;
    private final Affine affine;

    CustomGesturePaneSkin(CustomGesturePane control) {
        super(control);
        pane = getSkinnable();
        affine = pane.affine;
...
//		affine.setOnTransformChanged(e -> pane.fireAffineEvent(CHANGED));
    ...
    private void setupGestures() {
        ...
        pane.addEventHandler(MouseEvent.MOUSE_DRAGGED,
                consumeThenFireIfEnabled(e -> {
                    if (pane.panningMouseButtons.contains(e.getButton())) {
